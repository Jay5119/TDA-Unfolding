...
class LISTA_visu(nn.Module):
    def __init__(self, m, n, Dict, numIter, L, device):
        ...
        self.fc1 = nn.Linear(m + Dict.shape[0] * Dict.shape[1] + 1, 512)
    ...
    def forward(self, y, sigma):
        ...
        x = torch.cat((x, sigma), dim = 1).float()
...
X = datagenerator.generate_sparse_signal(numTrain) #genarating sparse samples
...
net_mixed = LISTA_train(X_train_mixed, Y_train_mixed, X_val_mixed, Y_val_mixed, A, 150, numLayers, device, learning_rate, batch_size, sigma_train_mixed, sigma_val_mixed)
...
torch.save(net_mixed, "new_epoch_150_lr_7e_5.pth")
