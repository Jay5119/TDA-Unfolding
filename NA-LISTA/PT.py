...
def LISTA_train(X, Y, X_val, Y_val, D, numEpochs, numLayers, device, learning_rate, batch_size):
    ...
    output_dir = "plots" #plotting train and val curves
...
def data(sigma): #splitting data to train, test, val
...
"""using the test data that was used for the other models"""
X1_test = np.load("broad_SNR/X1_test.npy")
Y1_test = np.load("broad_SNR/Y1_test.npy")
X2_test = np.load("broad_SNR/X2_test.npy")
Y2_test = np.load("broad_SNR/Y2_test.npy")
X3_test = np.load("broad_SNR/X3_test.npy")
Y3_test = np.load("broad_SNR/Y3_test.npy")
...
net_mixed_1 = LISTA_train(X1_train, Y1_train, X1_val, Y1_val, A, 215, numLayers, device, learning_rate, batch_size)
print(f'time taken is {time.time() - start}')

"""for analysis purposes"""
##############################
W = net_mixed_1._S.weight.detach().cpu().numpy()
max_val = np.max(np.abs(W))
W_norm = W/max_val
print("alpha_norm:", np.linalg.norm(W_norm, 'fro'))
###############################
...
torch.save(net_mixed_1, "new_epoch_215_lr_6e_5.pth")
