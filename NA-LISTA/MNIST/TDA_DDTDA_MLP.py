...
class DDTDAThresholdNet(nn.Module):
    """
    MLP-based threshold predictor with layer-wise (global) thresholds.
    Data-driven approach without explicit noise level.
    ...
    def forward(self, y_vec: torch.Tensor) -> torch.Tensor:
        B = y_vec.shape[0]
        dev = y_vec.device
        ...
        # Forward pass
        x = self.relu(self.fc1(y_flat))  # [B, 2*(M//3)]
        x = self.relu(self.fc2(x))       # [B, M//2]
        x = self.relu(self.fc3(x))       # [B, M//3]
...
class MultiChannelLISTALayer_DDTDA(nn.Module):
    """
    One untied multi-channel LISTA layer with DDTDA-provided thresholds.
...
class NA_LISTA_DDTDA(nn.Module):
    """
    K-layer multi-channel LISTA with thresholds predicted directly from y.
    ...
    def __init__(
        ...
        if tied:
            self.shared_layer = MultiChannelLISTALayer_DDTDA(
                M=M, N=N, A=A, C=C,
    ...
    def forward(self, Y: torch.Tensor, all_outputs: bool = False):
        ...
        for c in range(self.C):
            ...
            theta_c = self.thr_nets[c](y_c)  # [B, K, N]
