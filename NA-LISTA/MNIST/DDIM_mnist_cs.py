	...
	def __init__(self,
		...
		for level, mult in enumerate(channel_mult):
			for _ in range(num_res_blocks):
				layers = [Residual_block(ch, model_channels*mult, time_emb_dim, dropout)]
...
class GaussianDiffusion:
	...
	@torch.no_grad()
	def p_sample_loop(self, model, shape, clip_denoised=True):
		batch_size = shape[0]
		...
		for i in tqdm(reversed(range(0, self.timesteps)), desc='sampling loop time step', total=self.timesteps):
			img = self.p_sample(model, img, torch.full((batch_size,), i, device=device, dtype=torch.long), clip_denoised)
		return img

	# sample new images
	@torch.no_grad()
	def sample(self, model, image_size, batch_size=8, channels=3, clip_denoised=True):
	...
	def ddim_sample(
		...
		if init_noise is not None:
			sample_img = init_noise.to(device)
		else:
	...
	def ddim_sample_conditional(
		self,
		model,
		measurements,  # CS measurements: [B, 1, 28, 28]
		image_size=28,
		batch_size=8,
		channels=1,
		ddim_timesteps=50,
		...
		init_noise=None):
		"""
		CONDITIONAL DDIM sampling for compressed sensing reconstruction.
		Model is conditioned on measurements throughout the reverse process.
		"""
		# Make DDIM timestep sequence
		if ddim_discr_method == 'uniform':
			c = self.timesteps // ddim_timesteps
			...
			raise NotImplementedError(f'No ddim discretization method "{ddim_discr_method}"')
		...
		# Start from random noise or provided noise
		if init_noise is not None:
			sample_img = init_noise.to(device)
		else:
			sample_img = torch.randn((batch_size, channels, image_size, image_size), device=device)
		...
		for i in reversed(range(ddim_timesteps)):
			...
			# === CONDITIONING: Concatenate measurements ===
			x_cond = torch.cat([sample_img, measurements], dim=1)
			
			# Predict noise with conditioning
			pred_noise = model(x_cond, t)
			# ============================================
