...
def LISTA_train(X, Y, X_val, Y_val, numEpochs, numLayers, device, learning_rate, batch_size, A):
    ...
    for epoch in range(numEpochs):
        if epoch == round(numEpochs*0.5):
            ...
            for param_group in optimizer.param_groups:
              param_group['lr'] *= 0.1  # Reduce learning rate further
        else:
            pass
        T_tot_loss = 0
        net.train()
...
def data(PHI, mix_D_number, A, X, stddev_noise, m, n, k):
    datagenerator = datagen(n, m, k)
    X_train = []
    Y_train = []
    X_val = []
    Y_val = []
...
def run_experiment(learning_rate, numEpochs, numLayers, batch_size):
    ...
    print("-"*50)
    N_JT = X_train_mixed.shape[1] + X_val_mixed.shape[1] + X_test[0].shape[1]
    print("For JT model dataset size is: ", N_JT, flush=True)
    start = time.time()
    net_JT = LISTA_train(X_train_mixed, Y_train_mixed, X_val_mixed, Y_val_mixed, numEpochs, numLayers, device, learning_rate, batch_size, A = A)
    print(f'time taken is {time.time() - start}')
    # Save the model
    model_path = os.path.join(script_dir, "JT.pth")
    torch.save(net_JT.state_dict(), model_path)
    print(f"Saved model for JT at {model_path}", flush=True)
    for name, param in net_JT.named_parameters():
        if param.requires_grad:
            print(f"Trainable Parameter: {name}", flush=True)
        else:
            print(f"Non-Trainable Parameter: {name}", flush=True)
    ...
    for i in range(len(X_test)):
        mse, hr = evaluate_lista_per_sample(net_JT, Y_test[i], X_test[i], A, device)
        mse_list.append(mse)
        hr_list.append(hr)
    # Then, evaluate the 5 "X_test_gen" and "Y_test_gen" pairs
    for i in range(len(X_test_gen)):
        mse, hr = evaluate_lista_per_sample(net_JT, Y_test_gen[i], X_test_gen[i], A, device)
        mse_list.append(mse)
        hr_list.append(hr)
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(script_dir, "JT.txt")
    ...
    with open(output_path, "a", buffering=1) as f:
        ...
        ## Run:
        ##  python3  JT.py --learning_rate 0.0005 --numEpochs 250 --numLayers 10 --batch_size 64
