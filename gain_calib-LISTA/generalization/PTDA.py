...
class LISTA(nn.Module):
    ...
    def forward(self, y, gain):
        x = torch.relu(self.bn1(self.fc1(torch.cat((y, gain), dim = 1)))).float()
...
class dataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.X[idx, :], self.Y[idx, :], self.gain[idx, :]  # since the input to the datset is tensor


def LISTA_train(X, Y, gain, X_val, Y_val, gain_val, numEpochs, numLayers, device, learning_rate, batch_size, A):
    m = int(Y.shape[0])
    n = int(X.shape[0])
    ...
    gain_t = torch.from_numpy(gain.T)
    gain_t = gain_t.float().to(device)
    gain_val_t = torch.from_numpy(gain_val.T)
    gain_val_t = gain_val_t.float().to(device)
    dataset_train = dataset(X_t, Y_t, gain_t)
    dataset_valid = dataset(X_val_t, Y_val_t, gain_val_t)
    ...
    for epoch in range(numEpochs):
        ...
        for iter, data in enumerate(dataLoader_train):
            X_GT_batch, Y_batch, gain_batch = data
            X_batch_hat = net(Y_batch.float(), gain_batch.float())  # get the outputs
...
def LISTA_test(net, Y, D, device, gain):
    # convert the data into tensors
    Y_t = torch.from_numpy(Y.T)
    if len(Y.shape) <= 1:
        Y_t = Y_t.view(1, -1)
    ...
    gain_t = torch.from_numpy(gain.T)
    if len(gain.shape) <= 1:
        gain_t = gain_t.view(1, -1)
    gain_t = gain_t.float().to(device)
    with torch.no_grad():
        # Compute the output
        net.eval()
        X_lista = net(Y_t.float(), gain_t.float())
        if len(Y.shape) <= 1:
...
class datagen():
    ...
    def data_gen(self, A, X, c, sigma_lst):
        Y = []
        C = []
        for sigma in sigma_lst:
            ...
            for _ in range(X.shape[1]):
             C.append(c_vector)
        return X, np.array(Y), np.array(C)

def evaluate_lista_per_sample(net, Y_test, gain_test, X_test_true, A, device, g=0.3):
    # Run LISTA network inference
    X_est = LISTA_test(net, Y_test, A, device, gain_test)
    # Convert to tensors
...
def data(PHI, mix_D_number, A, X, stddev_noise, m, n, sparsity):
    ...
    for d in range(mix_D_number):
        phi = np.diag(PHI[:, d])
        x_tem, y_tem, gain_tem = datagenerator.data_gen(np.matmul(phi, A), X, np.diagonal(phi), [stddev_noise]) 
        x_tem = x_tem.T
        y_tem = y_tem[0].T
        gain_tem = gain_tem
        x_train_val, x_test, y_train_val, y_test, gain_train_val, gain_test = train_test_split(x_tem, y_tem, gain_tem, test_size=0.2, random_state=42)  #train test val split
        x_train, x_val, y_train, y_val, gain_train, gain_val = train_test_split(x_train_val, y_train_val, gain_train_val, test_size=0.3, random_state=42)
        x_train, x_val, x_test = x_train.T, x_val.T, x_test.T
        y_train, y_val, y_test = y_train.T, y_val.T, y_test.T
        gain_train, gain_val, gain_test = gain_train.T, gain_val.T, gain_test.T
        X_train.append(x_train)
        Y_train.append(y_train)
        Gain_train.append(gain_train)
        X_val.append(x_val)
        Y_val.append(y_val)
        Gain_val.append(gain_val)
        X_test.append(x_test)
        Y_test.append(y_test)
        Gain_test.append(gain_test)

    return X_train, Y_train, Gain_train, X_val, Y_val, Gain_val, X_test, Y_test, Gain_test

def data_mix(seed, X, Y, Gain):
    l = X[0].shape[1]
    sel = (l // len(X))
    ...
    for x, y, g in zip(X, Y, Gain):
        ...
        gain_mixed_list.append(g_selected)
    # concatenate all selected samples
    x_mixed = np.concatenate(x_mixed_list, axis=1)
    y_mixed = np.concatenate(y_mixed_list, axis=1)
    gain_mixed = np.concatenate(gain_mixed_list, axis=1)
    # shuffle along the sample axis (same shuffling for all)
    indices = np.arange(x_mixed.shape[1])
    seed.shuffle(indices)  # <-- Use the given random generator for shuffling
    X_mixed = x_mixed[:, indices]
    Y_mixed = y_mixed[:, indices]
    Gain_mixed = gain_mixed[:, indices]

    return X_mixed, Y_mixed, Gain_mixed
...
def run_experiment(learning_rate, numEpochs, numLayers, batch_size):
    ...
    stddev_noise = 0.005
    N_PT = 5*N
    X_train, Y_train, Gain_train, X_val, Y_val, Gain_val, X_test, Y_test, Gain_test = data(PHI, mix_D_number, A, X[:,N_PT:], stddev_noise, m, n, sparsity)
    ...
    X_train_mixed, Y_train_mixed, Gain_train_mixed = data_mix(np_seed_mixed, X_train, Y_train, Gain_train)
    X_val_mixed, Y_val_mixed, Gain_val_mixed = data_mix(np_seed_mixed, X_val, Y_val, Gain_val)
    ...
    print("-"*50)
    N_PTDA = X_train_mixed.shape[1] + X_val_mixed.shape[1] + X_test[0].shape[1]
    print("For P-TDA model dataset size is: ", N_PTDA, flush=True)
    start = time.time()
    net_PTDA = LISTA_train(X_train_mixed, Y_train_mixed, Gain_train_mixed, X_val_mixed, Y_val_mixed, Gain_val_mixed, numEpochs, numLayers, device, learning_rate, batch_size, A = A)
    print(f'time taken is {time.time() - start}', flush=True)
    # Save the model
    model_path = os.path.join(script_dir, "PTDA.pth")
    torch.save(net_PTDA.state_dict(), model_path)
    print(f"Saved model for PTDA at {model_path}", flush=True)
    PHI_rest = PHI[:, mix_D_number:]
    rest_D_number = PHI_rest.shape[1]
    _, _, _, _, _, _, X_test_gen, Y_test_gen, Gain_test_gen = data(PHI_rest, rest_D_number, A, X, stddev_noise)
    for i in range(rest_D_number):
        clean_signal = np.matmul(A, X_test_gen[i])
        signal_power = np.mean(clean_signal**2)
        # Extracting the noise component
        noise = Y_test_gen[i] - clean_signal
        noise_power = np.mean(noise**2)
        # Compute SNR in dB
        snr = 10 * np.log10(signal_power / noise_power)
        print(f"Domian {i+mix_D_number+1} snr: {snr}", flush=True)
    mse_list = []
    hr_list = []
    # First, evaluate the 10 "X_test" and "Y_test" pairs
    for i in range(len(X_test)):
        mse, hr = evaluate_lista_per_sample(net_PTDA, Y_test[i], Gain_test[i], X_test[i], A, device)
        mse_list.append(mse)
        hr_list.append(hr)
    # Then, evaluate the 5 "X_test_gen" and "Y_test_gen" pairs
    for i in range(len(X_test_gen)):
        mse, hr = evaluate_lista_per_sample(net_PTDA, Y_test_gen[i], Gain_test_gen[i], X_test_gen[i], A, device)
        mse_list.append(mse)
        hr_list.append(hr)
    # Now, printing nicely
    ...
    for idx, (mse, hr) in enumerate(zip(mse_list, hr_list), 1):
        print(f"{'Domain ' + str(idx):<12} | {mse:>10.1f} | {hr:>10.1f}")
    print("-" * 45)
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(script_dir, "PTDA.txt")
...
## Run:
##  python3 PTDA.py --learning_rate 0.0005 --numEpochs 250 --numLayers 10 --batch_size 64
