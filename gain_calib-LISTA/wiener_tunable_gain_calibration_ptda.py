# -*- coding: utf-8 -*-
"""Wiener_tunable_gain_calibration_PTDA.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1rkGMmnLT4ayZxCs0BLpP4kV-GMFZdvvE
"""
...
class LISTA(nn.Module):
    ...
    def forward(self, y, sigma):
        ...
        """model"""
        x = torch.relu(self.bn1(self.fc1(torch.cat((y, sigma), dim = 1)))).float()
        ...
        diag = torch.relu(self.fc4(x).float())


        y_corrected = torch.mul(y, diag)
...
def LISTA_train(X, Y, sigma, X_val, Y_val, sigma_val, numEpochs, numLayers, device, learning_rate, batch_size, A):
...
class datagen():
    ...
    def random_sinusoidal_diagonal_matrix(self, m, frequency, Amp):
        ...
        diag_values = Amp*(np.sin(2 * np.pi * frequency * random_t) + 2)
...
seed = 80
np.random.seed(seed)
# np_seed = np.random.RandomState(400)
...
X = datagenerator.generate_sparse_signal(numTrain) #genarting sparse signals
...
def data(phi1, stddev_noise):
    X1, Y1, sigma1 = datagenerator.data_gen(np.matmul(phi1, A), X, np.diagonal(phi1), [stddev_noise]) #X1 corresponds to the sparse signals X, Y is the corresponding output, sigma1 represents the C array
    ...
    print(X1.shape, Y1.shape, sigma1.shape)

    X1_train_val, X1_test, Y1_train_val, Y1_test, sigma1_train_val, sigma1_test = train_test_split(X1, Y1, sigma1, test_size=0.2, random_state=42)  #train test val split
...
X1_train, Y1_train, sigma1_train, X1_val, Y1_val, sigma1_val, X1_test, Y1_test, sigma1_test = data(phi1, 0.005)

X2_train, Y2_train, sigma2_train, X2_val, Y2_val, sigma2_val, X2_test, Y2_test, sigma2_test = data(phi2, 0.005)

X3_train, Y3_train, sigma3_train, X3_val, Y3_val, sigma3_val, X3_test, Y3_test, sigma3_test = data(phi3, 0.005)
...
net_mixed = LISTA_train(X_train_mixed, Y_train_mixed, sigma_train_mixed, X_val_mixed, Y_val_mixed, sigma_val_mixed, numEpochs, numLayers, device, learning_rate, batch_size, A = A)

print(f'time taken is {time.time() - start}')

# torch.save(net_mixed, "/content/drive/My Drive/Weiner/random_close_to_zero/PTDA_epoch_100_lr_5e_5.pth")

#testing mse and HR metric function
def test_mse_hr1_hr2(net_mixed_1, Y1_test, A, device, X1_test, sigma1_test):
  g = 0.3
  X_est_5 = LISTA_test(net_mixed_1, Y1_test, A, device, sigma1_test)
...
test_mse_hr1_hr2(net_mixed, Y1_test, A, device, X1_test, sigma1_test)

test_mse_hr1_hr2(net_mixed, Y2_test, A, device, X2_test, sigma2_test)

test_mse_hr1_hr2(net_mixed, Y3_test, A, device, X3_test, sigma3_test)